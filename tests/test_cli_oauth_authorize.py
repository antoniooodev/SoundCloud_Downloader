...
def test_existing_policy_cli_still_works() -> None:
    result = CliRunner().invoke(app, ["policy", "evaluate", "--access-mode", "public"])

    assert result.exit_code == 0, result.output
    assert json.loads(result.output)["decision"] == "deny_source_not_downloadable"


def test_existing_plan_cli_still_works() -> None:
    result = CliRunner().invoke(
        app,
        ["plan", "evaluate", "--access-mode", "public", "--source-protocol", "download"],
    )
...
def test_existing_resolver_cli_still_works() -> None:
    result = CliRunner().invoke(app, ["resolver", "inspect", "https://soundcloud.com/user/track"])

    assert result.exit_code == 0, result.output
    assert json.loads(result.output)["normalized"]["input_type"] == "url"


def test_oauth_authorize_url_exits_zero_with_client_id_and_localhost_redirect_uri() -> None:
    result = _invoke_authorize_url()
...
def test_plain_output_prints_only_url_like_text_and_no_json_object() -> None:
    result = _invoke_authorize_url("--plain")

    assert result.exit_code == 0, result.output
    output = result.output.strip()
    assert output.startswith("https://secure.soundcloud.com/authorize?")
...
def test_invalid_verifier_length_exits_non_zero() -> None:
    result = _invoke_authorize_url("--verifier-length", "42")

    assert result.exit_code != 0


def test_redirect_uri_with_fragment_exits_non_zero() -> None:
    result = _invoke_authorize_url("--redirect-uri", "http://localhost:8765/callback#fragment")

    assert result.exit_code != 0


def test_auth_base_url_with_query_exits_non_zero() -> None:
    result = _invoke_authorize_url("--auth-base-url", "https://secure.soundcloud.com?bad=true")
...
def test_no_test_performs_network_calls(monkeypatch: Any) -> None:
    def fail_network_call(*args: object, **kwargs: object) -> None:
        raise AssertionError("Network calls are not allowed in OAuth CLI tests.")
...
def test_no_test_writes_files_except_explicit_tmp_path_env_file(monkeypatch: Any) -> None:
    def fail_file_write(*args: object, **kwargs: object) -> None:
        raise AssertionError("File writes are not allowed in OAuth CLI tests.")
...
def _invoke_authorize_url(*extra_args: str):
    args = [
        "oauth",
        "authorize-url",
        "--client-id",
...
def _authorize_payload(*extra_args: str) -> dict[str, Any]:
    result = _invoke_authorize_url(*extra_args)
    assert result.exit_code == 0, result.output
    return json.loads(result.output)


def _parsed_authorization_url(*extra_args: str):
    return urlparse(_authorize_payload(*extra_args)["authorization_url"])
